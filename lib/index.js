...
class RuleVisitor extends Visitor2 {
  ImportHandler;
  CallHandler;
...
program.name("deplister").description("list dependency for specified ts(x)/js(x) files").argument("[folder...]", "folder to deplistering").option("--aggregated", "aggregated results").option("--cleanResult", "clean result").option("--skipImport", "skip parse import").option("--preset <preset>", "preset", checkPreset).option("--config <config>", "config", checkConfig).option("-y, --yaml", "output to yaml").option("-f, --filename <name>", "default filename").option("--allowed <ext...>", "allowed extenstions").option("--notallowed <ext...>", "ignored extension in deplistering folders").option("--ignore <folder...>", "folders to be ignored").version(JSON.parse(fs2.readFileSync(path2.join(__dirname, "../package.json")).toString()).version).action((folder, options) => {
  ...
  if (config.aggregated) {
    dependencies = aggregateDependency(dependencies);
